...
var _upload = function (file, opt, callback) {
    var _req = this.request("post", COS_API.upload, opt, {
        headers: {
            "content-length": file?fs.statSync(file).size:opt.size
        }
    }, function (error, data) {
        callback && callback(null, data);
    });

    file && fs.createReadStream(file).pipe(_req);
...
function upload(bucketId, file, opt, callback) {
    ...
    return _upload.apply(this, args);
}
...
function convertAfterUpload(file, opt, callback) {
    ...
    var _req = this.request("post", COS_API.compressUpload, opt, {
        headers: {
    ...
    fs.createReadStream(file).pipe(_req);
    return _req;
}
...
Convert.prototype.crop = function (x, y, width, height) {
    this._opt.compress = 0; //如果进行裁剪，暂时把compress参数去掉。
...
Convert.prototype.to = function (bucketUrl, callback) {
    ...
    if (this.local) { //上传并压缩文件
        ...
        return convertAfterUpload.call(this.cosObj, this.local, opt, callback);
    } else { //压缩线上文件
        var opt = {
        ...
        return convert.call(this.cosObj, opt, callback);
    }
